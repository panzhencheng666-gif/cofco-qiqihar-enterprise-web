...
export function createLocalStorageBusinessReportRepository(
  ...
  storageKey = prototypeBusinessReportStorageKey,
): BusinessReportRepository {
  return {
    load: () => {
...
export function createPrototypeBusinessReportSeeds(): readonly BusinessReportRecord[] {
...
export function createPrototypeBusinessReportWorkflow(
  storage?: BusinessReportStorage,
  ...
  try {
    const storedValue = storage.getItem(prototypeBusinessReportStorageKey);
...
export function getFallbackPrototypeBusinessReportWorkflow() {
  if (!fallbackPrototypeWorkflow) {
    const storage =
      typeof window === "undefined" ? undefined : window.localStorage;
    fallbackPrototypeWorkflow = createPrototypeBusinessReportWorkflow(storage);
  }
  return fallbackPrototypeWorkflow;
}
