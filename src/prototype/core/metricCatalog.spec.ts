...
describe("MetricDefinition", () => {
  it("keeps regional yield governed as a ratio of aggregates", () => {
    expect(validateMetricDefinition(definition()).aggregation).toBe(
      "ratio-of-aggregates",
    );
  });
  ...
  it("validates definition display scales as non-negative safe integers", () => {
    expect(() =>
      validateMetricDefinition(definition({ displayScale: 1.5 })),
    ).toThrow("指标显示精度无效");
  });
  ...
  it.each([
    ...
    "rejects whitespace-only governed definition field %s",
    (field, reason) => {
      expect(() =>
        validateMetricDefinition(definition({ [field]: "   " })),
      ).toThrow(reason);
    },
  );
  ...
  it("rejects blank price statistics, comparison IDs, and metric/domain disagreement", () => {
    ...
    expect(() =>
      validateMetricDefinition(definition({ priceStatisticId: "\t" })),
    ).toThrow("价格统计口径不能为空");
    expect(() =>
