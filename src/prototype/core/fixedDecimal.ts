...
function roundedRational(numerator: bigint, denominator: bigint, scale: number): FixedDecimal {
  assertScale(scale);
...
export function addFixedDecimal(left: FixedDecimal, right: FixedDecimal): FixedDecimal {
...
export function subtractFixedDecimal(left: FixedDecimal, right: FixedDecimal): FixedDecimal {
  const [leftCoefficient, rightCoefficient, scale] = align(parse(left), parse(right));
  return serialize({ coefficient: leftCoefficient - rightCoefficient, scale });
}

export function multiplyFixedDecimal(left: FixedDecimal, right: FixedDecimal): FixedDecimal {
...
export function compareFixedDecimal(left: FixedDecimal, right: FixedDecimal): -1 | 0 | 1 {
...
export function absFixedDecimal(value: FixedDecimal): FixedDecimal {
  ...
  return serialize({ ...parsed, coefficient: parsed.coefficient < 0n ? -parsed.coefficient : parsed.coefficient });
}

export function roundHalfUp(value: FixedDecimal, displayScale: number): FixedDecimal {
  assertScale(displayScale);
  const parsed = parse(value);
  if (parsed.scale <= displayScale) return serialize(parsed);
  return roundedRational(parsed.coefficient, powerOfTen(parsed.scale), displayScale);
}

export function formatFixedDecimal(value: FixedDecimal, displayScale: number): string {
  assertScale(displayScale);
...
export function percentageChange(
  ...
  const currentCoefficient = currentValue.coefficient * powerOfTen(commonScale - currentValue.scale);
  const baselineCoefficient = baselineValue.coefficient * powerOfTen(commonScale - baselineValue.scale);
  return roundedRational(
...
export function cagrPercent(
  ...
  const ratioNumerator = currentValue.coefficient * powerOfTen(baselineValue.scale);
  const ratioDenominator = baselineValue.coefficient * powerOfTen(currentValue.scale);
  ...
  for (let precision = displayScale + 8; ; precision += 8) {
    const floor = floorScaledRoot(ratioNumerator, ratioDenominator, years, precision);
    const candidate = cagrCandidate(floor, precision, displayScale);
    if (candidateIsCertified(ratioNumerator, ratioDenominator, years, candidate, displayScale)) {
