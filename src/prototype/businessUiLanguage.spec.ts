...
const forbiddenBusinessUiTerms =
  /后端|本地数据库|演示数据|开发模式|调试信息|API\s|接口地址|模板版本|数据版本|结果版本|来源版本/u;

describe("正式业务界面用语", () => {
  it("不把开发、运行和内部版本信息暴露到业务组件", () => {
    const files = componentFiles(join(process.cwd(), "src/prototype"));
...
function componentFiles(directory: string): string[] {
  return readdirSync(directory, { withFileTypes: true }).flatMap((entry) => {
    ...
    if (
      ...
      entry.name === "EnterpriseArchitecturePrototype.tsx"
    ) {
      return [];
    }
    return [path];
