...
const cornProductionDocument: ProductionDocumentFixture = {
...
function createProductProductionDocument({
  documentId,
  workId,
  productLabel,
  cultivar,
  ...
  cultivar: string;
  objectName: string;
  qualityFields: readonly ProductionDocumentField[];
  lastSavedLabel: string;
}): ProductionDocumentFixture {
  return {
    ...cornProductionDocument,
    documentId,
    workId,
    documentLabel: `第 31 周${productLabel}产情调查单`,
    objectName,
    lastSavedLabel,
    fieldGroups: cornProductionDocument.fieldGroups.map((group) => {
      if (group.groupId === "specific-variety") {
        return {
          ...group,
          fields: group.fields.map((field) =>
            field.fieldId === "cultivar"
              ? { ...field, value: cultivar }
              : field,
          ),
        };
      }
      if (group.groupId === "area-location") {
        return {
          ...group,
          fields: group.fields.map((field) =>
            field.fieldId === "location"
              ? { ...field, value: objectName }
              : field,
          ),
        };
      }
...
export const productionDocumentFixtures: readonly ProductionDocumentFixture[] =
  [
    cornProductionDocument,
    createProductProductionDocument({
      documentId: "DOCUMENT-PRODUCTION-SOY-W31-01",
      workId: "WORK-PRODUCTION-SOY-W31",
      productLabel: "大豆",
      cultivar: "黑农84",
      objectName: "讷河市同义镇保国村村委会",
      lastSavedLabel: "2026 年 7 月 31 日 14:18",
      qualityFields: [
        { fieldId: "protein", label: "蛋白", value: "39.6%" },
        { fieldId: "oilYield", label: "出油率", value: "18.7%" },
        { fieldId: "imperfectGrain", label: "不完善粒", value: "1.9%" },
        { fieldId: "moisture", label: "水分", value: "12.8%" },
        { fieldId: "impurity", label: "杂质", value: "0.7%" },
      ],
    }),
    createProductProductionDocument({
      documentId: "DOCUMENT-PRODUCTION-PADDY-W31-01",
      workId: "WORK-PRODUCTION-PADDY-W31",
      productLabel: "稻谷",
      cultivar: "龙粳31",
      objectName: "讷河市同义镇农技站",
      lastSavedLabel: "2026 年 7 月 31 日 14:12",
      qualityFields: [
        { fieldId: "moisture", label: "水分", value: "15.2%" },
        { fieldId: "milledRiceRate", label: "出米率", value: "68.1%" },
        { fieldId: "brownRiceRate", label: "出糙率", value: "78.4%" },
        { fieldId: "impurity", label: "杂质", value: "0.8%" },
      ],
    }),
  ];
