...
describe("useFormalEnterpriseLocation", () => {
  it("migrates an old English query to a Chinese-only business hash", () => {
    window.history.replaceState(
      {},
      "",
      "/prototype.html?variant=A&page=overview&section=operations&region=qiqihar-nehe&product=corn",
    );

    render(<LocationProbe />);

    expect(window.location.pathname).toBe("/prototype.html");
  ...
  it("keeps business state in history state and exposes only Chinese module and view names", async () => {
    window.history.replaceState({}, "", "/prototype.html#/市场监测/监测对象");
  ...
  it("restores coordinates and selection across back and forward history entries", async () => {
    window.history.replaceState({}, "", "/prototype.html#/我的工作/我的任务");
    const user = userEvent.setup();
    render(<LocationProbe />);
  ...
  it("ignores injected identifiers and canonicalizes invalid or legacy routes", () => {
    window.history.replaceState(
      {},
      "",
      "/prototype.html?page=market&section=analysis&region=qiqihar-nehe&product=corn&period=2026-W31&selectionType=object&selectionId=MARKET-001",
    );
    render(<LocationProbe />);
    ...
    window.history.replaceState(
      {},
      "",
      "/prototype.html#/不存在的模块/INTERNAL-001",
    );
    void act(() => {
  ...
  it("keeps saved views in state without exposing their identifiers", async () => {
    window.history.replaceState({}, "", "/prototype.html#/市场监测/市场分析");
    const replaceState = window.history.replaceState.bind(window.history);
