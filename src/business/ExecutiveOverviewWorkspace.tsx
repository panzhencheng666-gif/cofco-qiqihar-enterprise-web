...
function realtimeExecutiveOptions(
  ...
  for (const item of workItems) {
    ...
    if (
      item.regionId &&
      item.regionLabel.trim() &&
      !regions.has(item.regionId)
    ) {
    ...
    if (
      ...
      classifications.set(item.businessSubtypeId, {
        ...
        label:
          item.businessLabel.trim() ||
          domainLabels.get(item.domain) ||
          "业务事项",
        domain: item.domain,
      });
    }
        ...
        {showProduct && (
          <label>
            ...
            <select
              aria-label="产品或作物"
              value={
                productInvalid ? "__invalid-product__" : (query.productId ?? "")
              }
              onChange={(event) =>
                onScopeChange({
...
export function ExecutiveOverviewWorkspace({
  ...
  const coordinateIssues = getExecutiveScopeCoordinateIssues(
    queryScope,
    realtimeMode ? realtimeOptions.periods.map(({ id }) => id) : undefined,
  );
