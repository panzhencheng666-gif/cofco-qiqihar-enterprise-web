...
.realtime-supply-panel[data-dashboard="supply"]
  ...
  it("switches real curves to lines, retains gaps and reads updated monthly values", () => {
    const lines = [{ key: "output", label: "预计总产", unit: "吨",
      value: (snapshot: ReturnType<typeof validSnapshot>) => snapshot.production.metrics.find(({ code }) => code === "EXPECTED_OUTPUT")?.value ?? null }];
    const { container, rerender } = render(<AnalysisTrendChart title="总产变化" lines={lines}
      points={[point(1, "10"), point(2, "20"), point(4, "40")]} />);
    const path = () => container.querySelector('[data-series-key="output"] path')?.getAttribute("d");
    ...
    expect(container.querySelector('.analysis-live-trend__readout')).toHaveTextContent("预计总产：20.00 吨");
    rerender(<AnalysisTrendChart title="总产变化" lines={lines}
      points={[point(1, "10"), point(2, "25"), point(4, "40")]} />);
    expect(container.querySelector('.analysis-live-trend__readout')).toHaveTextContent("预计总产：25.00 吨");
  });
