...
function point(year: number, value: string): PublishedMetricPoint {
  return {
    ...
    coordinate: {
      ...
      regionBoundaryVersionId: "authorized-membership-v1",
      cropId: "corn",
      commodityId: null,
      productFormId: "grain",
      ...
      populationOrSampleId: "regional",
      unitDefinitionVersionId: "unit-v1",
      inventoryNatureId: null,
      statisticalMomentId: "annual-final",
      consolidationMatrixVersionId: null,
      domainDimensions: {
        domain: "production",
        areaBasisId: "harvested",
        yieldMethodId: "weighted",
        growthStageId: null,
        surveyRoundId: "final",
...
describe("MetricComparisonViewModel", () => {
  it("keeps canonical values separate from rounded display strings and release labels", () => {
    const model = createMetricComparisonViewModel(
      definition,
      comparison([
        point(2023, "100"),
        point(2024, "110"),
    ...
    expect(model).toMatchObject({
      ...
      unit: "万吨",
      currentValue: "133.1",
      currentChangeText: "10.0%",
      cagrText: "年均复合增长率 10.0%",
      comparabilityText: "四年口径连续可比",
    });
    expect(model.yearCells[0]).toEqual({
      year: 2023,
      valueText: "100.0",
      availabilityLabel: "可用",
      releaseVersionLabel: "metric-2023-v1",
      reason: null,
    });
    expect(model.levelSeries[0]).toEqual({
      year: 2023,
      rawValue: fixedDecimal("100"),
      valueText: "100.0",
    });
    expect(model.annualChangeSeries[2]).toMatchObject({
      label: "当前同比",
      changeKind: "relative-rate",
      rawChange: fixedDecimal("10"),
      changeText: "10.0%",
      reason: null,
    });
    expect(
      model.currentVsBaselineSeries.map(({ label, rawChange, changeText }) => ({
        label,
        rawChange,
        changeText,
      })),
      ...
      {
        label: "较 2023 年变化",
        rawChange: fixedDecimal("33.1"),
        changeText: "33.1%",
      },
      {
        label: "较 2024 年变化",
        rawChange: fixedDecimal("21"),
        changeText: "21.0%",
      },
      {
        label: "较 2025 年变化",
  ...
  it("preserves unavailable state and its governed Chinese reason without breaking unrelated pairs", () => {
    ...
    expect(model.yearCells[1]).toEqual({
      year: 2024,
      valueText: "未采集",
    ...
    expect(model.pairCells[0]).toMatchObject({
      state: "not-comparable",
      reason: "本年度未组织采集",
      changeText: "本年度未组织采集",
    });
    expect(model.pairCells[2]).toMatchObject({
      state: "comparable",
      reason: null,
      changeText: "10.0%",
    });
    expect(model.levelSeries[1]).toEqual({
      year: 2024,
      rawValue: null,
      valueText: "未采集",
    });
  ...
  it("keeps an unavailable year reason while pair governance resolves its definition", () => {
    ...
    const unbridged = createMetricComparisonViewModel(
      definition,
      comparison(points),
    );
    expect(unbridged.yearCells[1].reason).toBe("本年度缺失");
    expect(unbridged.pairCells[0]).toMatchObject({
      state: "not-comparable",
      reason: "各年度统计口径尚未完成可比性确认",
    });
    expect(JSON.stringify(unbridged)).not.toMatch(
      /版本|桥接|治理|METRIC|BUILD/,
    );

    const bridged = createMetricComparisonViewModel(
      definition,
      comparison(points, [
        {
          metricId: definition.metricId,
    ...
    expect(bridged.pairCells[0]).toMatchObject({
      state: "not-comparable",
      reason: "本年度缺失",
      changeText: "本年度缺失",
    });
  ...
  it("preserves formula-unavailable reasons while coordinates remain comparable", () => {
    ...
    expect(model.pairCells[2]).toEqual({
      label: "当前同比",
  ...
  it("distinguishes a release attempt without a metric release from no attempt", () => {
    const attempted: PublishedMetricPoint = {
      ...
      coordinate: point(2024, "0").coordinate,
      releaseAttempt: {
        inputReleaseVersionIds: ["facts-2024-attempt"],
        metricReleaseVersionId: null,
      },
      value: null,
      unit: definition.unit,
    ...
    expect(model.yearCells[1].releaseVersionLabel).toBe(
      "已发起，未形成指标发布",
    );
  });
});
