...
import { businessComparisonReason } from "./businessDisplayPolicy";
...
function releaseLabel(point: PublishedMetricPoint): string {
  if (point.availability === "available")
    return point.coordinate.metricReleaseVersionId;
  if (point.releaseAttempt === null) return "未发起发布";
  return (
    point.releaseAttempt.metricReleaseVersionId ?? "已发起，未形成指标发布"
  );
}

function changeKind(
  definition: MetricDefinition,
): MetricComparisonViewModel["annualChangeSeries"][number]["changeKind"] {
...
function rawChange(
  ...
  if (definition.comparisonPolicy.relativeChange === "allowed")
    return pair.relativeRate;
  if (definition.comparisonPolicy.relativeChange === "percentage-points")
    return pair.percentagePointDelta;
...
function changeText(
  ...
  if (change === null)
    return businessComparisonReason(pair.reason) ?? "比较数据暂不可用";
  const formatted = formatFixedDecimal(change, definition.displayScale);
  if (definition.comparisonPolicy.relativeChange === "allowed")
    return `${formatted}%`;
  if (definition.comparisonPolicy.relativeChange === "percentage-points")
    return `${formatted} 个百分点`;
...
export function createMetricComparisonViewModel(
  ...
  if (comparison.metricId !== definition.metricId)
    throw new Error("比较结果与指标定义不一致");
  const current = comparison.points[3];
  const pairCells = comparison.pairs.map((pair) => ({
    label: pair.label,
    changeText: changeText(pair, definition),
    state: pair.comparable
      ? ("comparable" as const)
      : ("not-comparable" as const),
    reason: businessComparisonReason(pair.reason),
  }));
  return {
    ...
    yearCells: comparison.points.map((point) => ({
      ...
      reason:
        point.availability === "available"
          ? null
          : businessComparisonReason(point.reason),
    })),
    pairCells,
    cagrText:
      comparison.cagr.status === "available"
        ? `年均复合增长率 ${formatFixedDecimal(comparison.cagr.rate, definition.displayScale)}%`
        : (businessComparisonReason(comparison.cagr.reason) ??
          "复合增长率暂不可用"),
    comparabilityText:
      comparison.trend.continuity === "continuous" &&
      comparison.pairs.every(({ comparable }) => comparable)
        ? "四年口径连续可比"
