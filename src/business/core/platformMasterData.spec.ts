...
import {
  ...
  platformMasterData,
  fixtureMasterDataAuthorization,
...
describe("platform master data", () => {
  ...
  it("provides a multi-region multi-product default authorization with only applicable cultivars and batches", () => {
    ...
    expect(fixtureMasterDataAuthorization.authorizedRegionIds.length).toBe(
      governedRegionIds.length,
    );
    expect(fixtureMasterDataAuthorization.authorizedRegionIds).toEqual(
      governedRegionIds,
    );
    expect(fixtureMasterDataAuthorization.authorizedProductIds).toEqual(
    ...
    expect(
      fixtureMasterDataAuthorization.authorizedReleaseVersionIds.length,
    ).toBeGreaterThan(1);

    for (const cultivarId of fixtureMasterDataAuthorization.authorizedCultivarIds) {
      expect(
        fixtureMasterDataAuthorization.authorizedProductIds.some((productId) =>
          isCultivarApplicableToProduct(productId, cultivarId),
        ),
        cultivarId,
      ).toBe(true);
    }

    expect(fixtureOperationalIdentity.authorization).toMatchObject(
      fixtureMasterDataAuthorization,
    );
  });
});
