...
describe("enterprise metric fixtures", () => {
  ...
  it("normalizes an unambiguous short subtype only against full authorization", () => {
    const results = queryFixtureMetricComparisons({
      scope: scope({
        regionId: "qiqihar-all",
  ...
  it("denies invalid scope, unknown subtype, and unknown region without fallback", () => {
    ...
    const unknownResults = queryFixtureMetricComparisons({
      scope: unknownRegion,
      queryAllowed: true,
      domain: "production",
      currentYear: 2026,
  ...
  it("serves the exact concrete source scope and never falls back from an aggregate, another product, or a cultivar", () => {
    const concrete = queryFixtureMetricComparisons({
    ...
    const partial = queryFixtureMetricComparisons({
      scope: partialScope,
      queryAllowed: true,
      domain: "production",
      currentYear: 2026,
    ...
    const soybean = queryFixtureMetricComparisons({
      scope: soybeanScope,
      queryAllowed: true,
      domain: "production",
      currentYear: 2026,
    ...
    const cultivar = queryFixtureMetricComparisons({
  ...
  it("binds the selected authorized release to current Y while retaining historical lineage", () => {
    expect(fixtureCurrentMetricReleaseVersionId).toBe("METRIC-2026-W31-V3");
    const [result] = queryFixtureMetricComparisons({
      scope: scope({
        regionId: "qiqihar-all",
    ...
    const mismatchResults = queryFixtureMetricComparisons({
      scope: mismatch,
      queryAllowed: true,
      domain: "production",
      businessSubtype: "production.planting-production",
    ...
    const unauthorizedCurrent = queryFixtureMetricComparisons({
      scope: noSelector,
      queryAllowed: true,
      domain: "production",
      businessSubtype: "production.planting-production",
    ...
    const authorizedCurrent = queryFixtureMetricComparisons({
      scope: scope({ regionId: "qiqihar-all" }),
      queryAllowed: true,
      domain: "production",
      businessSubtype: "production.planting-production",
    ...
    const historicalOnlyResults = queryFixtureMetricComparisons({
  ...
  it("never returns agricultural-input series for a corn query", () => {
    ...
    const cornResults = queryFixtureMetricComparisons({
      scope: cornScope,
      queryAllowed: true,
      domain: "market",
      currentYear: 2026,
    ...
    const agriResults = queryFixtureMetricComparisons({
      scope: agriScope,
      queryAllowed: true,
      domain: "market",
      currentYear: 2026,
