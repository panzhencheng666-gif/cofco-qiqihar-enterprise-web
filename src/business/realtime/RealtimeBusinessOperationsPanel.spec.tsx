...
it("shows legacy preview field errors and clears the edited field", async () => {
  const { api, getProduction } = repository();
  const record = {
    id: "legacy-preview",
    productCode: "CORN",
    ...
    insurance: {},
    subsidies: {},
    submissionMetadata: { PROD_SAMPLE_NAME: "旧记录" },
    reportedAt: "2026-08-08T10:00:00+08:00",
    estimatedOutputKilograms: "65000",
    status: "PENDING_REVIEW",
    returnReason: null,
  ...
  getProduction.mockResolvedValue(record);
  api.getValidationPreview = vi.fn().mockResolvedValue({
    id: record.id,
    version: 1,
    fieldValidationPassed: false,
    code: "INVALID_FIELD",
    message: "请修正样本名称",
    details: { fieldErrors: { PROD_SAMPLE_NAME: "请修正样本名称" } },
  });
  render(
    <RealtimeBusinessOperationsPanel
      actorName="产情填报员"
  ...
  expect((await screen.findAllByText("请修正样本名称")).length).toBeGreaterThan(
    0,
  );
  const input = screen.getByDisplayValue("旧记录");
  expect(input).toHaveAttribute("aria-invalid", "true");
  fireEvent.change(input, { target: { value: "修正后的样本" } });
  expect(input).not.toHaveAttribute("aria-invalid", "true");
  expect(api.getValidationPreview).toHaveBeenCalledWith(
    "production",
    record.id,
  );
});
