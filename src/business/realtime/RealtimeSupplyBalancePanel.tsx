    ...
    >
      ...
      >
            ...
            >
              {accounts.map((account) => (
                <option key={account.id} value={account.id}>
                  {account.surveyQuarter ?? "年度"} · 第 {account.resultVersion}{" "}
                  版 ·{" "}
                  {businessLabel(
                    resultStateLabels,
                    account.resultState,
        ...
        <p className="enterprise-ledger-query__summary">
          ...
          {" · "}
          {surveyPeriods.find(
            ({ code }) => code === (activeAccount?.periodCode ?? periodCode),
          )?.name ?? "尚无调查期间"}
          {" · "}
          {activeAccount?.marketingYear
            ? `${activeAccount.marketingYear}营销年度`
            : (surveyPeriods.find(({ code }) => code === periodCode)
                ?.marketingYearName ?? `${marketingYear}营销年度`)}
          {" · "}
