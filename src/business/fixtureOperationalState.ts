...
export const fixtureOperationalStateStorageKey =
  "齐齐哈尔粮食商情业务工作状态-第一版";

export interface FixtureOperationalStateStorage {
  getItem(key: string): string | null;
  setItem(key: string, value: string): void;
}

export interface FixtureOperationalState {
...
export interface FixtureOperationalStateLoadResult {
  status: "empty" | "loaded" | "blocked";
  state: FixtureOperationalState;
  message?: string;
}

export type FixtureOperationalStateSaveResult =
  { status: "saved" } | { status: "blocked"; message: string };

interface StoredFixtureOperationalState {
  schemaVersion: 1;
  savedAt: string;
  state: FixtureOperationalState;
}
...
function isOperationalState(value: unknown): value is FixtureOperationalState {
  if (!isRecord(value)) return false;
  return (
...
export function createDefaultFixtureOperationalState(): FixtureOperationalState {
  return {
 ...
 * Keeping this separate from the default fixture state prevents a failed or empty
 * database query from being presented as business truth.
 */
export function createEmptyFixtureOperationalState(): FixtureOperationalState {
  return {
    workItems: [],
    marketDocumentDrafts: {},
    productionDocumentDrafts: {},
...
export function loadFixtureOperationalState(
  storage: FixtureOperationalStateStorage | undefined,
): FixtureOperationalStateLoadResult {
  const fallback = createDefaultFixtureOperationalState();
  if (!storage) return { status: "empty", state: fallback };
  let raw: string | null;
  try {
    raw = storage.getItem(fixtureOperationalStateStorageKey);
  } catch {
    return {
      status: "blocked",
      state: fallback,
...
export function saveFixtureOperationalState(
  storage: FixtureOperationalStateStorage | undefined,
  state: FixtureOperationalState,
): FixtureOperationalStateSaveResult {
  if (!storage) return { status: "saved" };
  const stored: StoredFixtureOperationalState = {
    schemaVersion: 1,
    savedAt: new Date().toISOString(),
    state,
  };
  try {
    storage.setItem(fixtureOperationalStateStorageKey, JSON.stringify(stored));
    return { status: "saved" };
  } catch {
    return {
      status: "blocked",
