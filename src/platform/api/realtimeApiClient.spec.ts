...
describe("realtime API client", () => {
  it("does not reuse an older in-flight read after a write", async () => {
    ...
    const fetcher = vi.fn<typeof fetch>()
      ...
      .mockImplementation(async () => new Response(JSON.stringify({data: {version: 2}})));
    ...
    await expect(client.get("/api/v1/example")).resolves.toEqual({version: 2});
  ...
  it("coalesces simultaneous reads without keeping settled business data", async () => {
    ...
    const client = createRealtimeApiClient({baseUrl: "", fetcher});
    await Promise.all([client.get("/api/v1/work-items", {page: 0}), client.get("/api/v1/work-items", {page: 0})]);
