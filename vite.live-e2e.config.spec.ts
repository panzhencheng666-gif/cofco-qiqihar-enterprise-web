...
describe("live E2E preview identity boundary", () => {
  ...
  it("proxies an expired session without injecting a local actor", () => {
    let proxyRequestHandler:
      | ((request: {
          removeHeader(name: string): void;
          setHeader(name: string, value: string): void;
    ...
    const config = createLiveE2eConfig({
      LIVE_E2E_AUTH_MODE: "anonymous",
      LIVE_E2E_API_TARGET: "http://127.0.0.1:63183",
    });
    const proxyOptions = config.preview?.proxy?.["/api"];
    expect(proxyOptions).toBeDefined();
    if (typeof proxyOptions === "string")
      throw new Error("Expected configured proxy options");
    proxyOptions?.configure?.(proxy as never, {});
    const request = {
      removeHeader: vi.fn(),
      setHeader: vi.fn(),
