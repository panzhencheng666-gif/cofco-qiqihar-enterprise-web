...
- 只允许批准 CIDR 访问 ECS 的 443；DNS A 记录必须包含云API确认属于目标 ECS 的批准 HTTPS 入口 IP，验证连接固定到该 IP；SSH 来源至少收窄到 `/24`，优先 `/32`，并通过绑定批准HostName/非root用户/0600或0400身份文件/已知主机密钥的 `~/.ssh/config` 别名，以非交互严格模式进入。
...
3. 输入齐全后只读云计划：`infra.sh plan <config>`。人工核对保存的 plan 及 SHA-256。
...
7. 远端部署先覆盖并回读命名预生产 RDS 白名单，再于拉取容器前核对 ECS/VPC/vSwitch/安全组、RDS 私网/无公网及 IPv4/IPv6 全局开放；随后执行 KMS 引用物化、RDS 手工备份并等待完成、镜像拉取、健康等待、TLS/匿名失败关闭/监控检查。任何失败自动恢复上一个已验证镜像；首次部署失败则停止服务并回到未部署状态。
